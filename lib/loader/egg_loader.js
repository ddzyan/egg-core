...
/**
 * Loader 父类，所有文件的 loader 都继承这个，内部实现 loader 文件时的一些必要的通用方法
 * 理解这个类的方法，可以通过具体的 loader 去分析
 */
class EggLoader {
  /**
   * @class
  ...
  /**
   * @description  获取服务环境变量，优先先获取构造函数配置，没有再获取config/env文件，没有获取启动环境环境变量
   ...
   * Get all loadUnit
   *
   * LoadUnit是一个可以由Eggloder加载的目录，它具有相同的结构。
   * LoadUnit有路径和类型（app，framework，plugin）。
   *
  ...
  // 将配置的路径内容所有方法挂载到 this.app 上
  loadToApp(directory, property, opt) {
    const target = this.app[property] = {};
    opt = Object.assign({}, {
      directory, // 加载文件夹路径
      target, // 目标对象
      inject: this.app, // 注入的的参数
    }, opt);
...
const loaders = [
  require('./mixin/plugin'), // 插件
  require('./mixin/config'), // 配置文件
  require('./mixin/extend'), // 继承文件
  require('./mixin/custom'), // 定制化
  require('./mixin/service'), // 服务
  require('./mixin/middleware'), // 中间件
  require('./mixin/controller'), // 控制器
  require('./mixin/router'), // 路由
  require('./mixin/custom_loader'), // 定制化加载器
];

// 每个 loader 都是一个对象，通过 Object.assign 进行浅拷贝，将对象的方法挂载到 EggLoader 对象上
// 这样做的好处是，只要实例化一个 EggLoader 对象就可以，而不需要每个 Loader 进行实例化（如果采用继承的方式）
