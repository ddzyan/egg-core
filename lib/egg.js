...
/**
 * Egg核心类，内部有三个重要属性：
 *   1. BaseContextClass : egg封装的基础上下文对象，内部的可以通过this获取上下文，可以导出 Controller 和 Service 的基础类，用于用户编写 controller 和 Service 时候基础，可以获得一些自带的属性（重点就是 this 对象）和方法
 *   2. lifecycle ：生命周期对象，用于在每个阶段执行不同的事情，其中最基础的就比如 router ，他需要在框架启动前作为中间件给 app.use 使用
 *   3. loader：最核心的加载对象，用以加载 controller ，service，router 等等，具体的功能请查看 loader 文件件下
 */
class EggCore extends KoaApplication {
   ...
   * @param {String} [options.baseDir=process.cwd()] - 应用根路径
   * @param {String} [options.type=application|agent] - 无论是运行在 app work 还是 agent work 中
   * @param {Object} [options.plugins] - 定制化插件
   * @since 1.0.0
   */
  constructor(options = {}) {
    ...
    // 使用 egg 封装的输出，避免直接使用 console 造成在控制台输出
  ...
  // 重写覆盖Koa的use方法，在将中间价添加到内部的 middleware 之前做了一次定制化转换
  use(fn) {
  ...
  // 通过 this.router 获取 router 对象的时候触发，并且在框架启动前通过中间件加载
  get router() {
    if (this[ROUTER]) {
      return this[ROUTER];
    }
